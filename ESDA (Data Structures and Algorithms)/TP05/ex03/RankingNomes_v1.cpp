...
{
  if(nome == "" || posicao <= 0 || posicao > (int)nomes.size()) return -1;
...
//Este método produz os resultados desejados segundo o trabalho, apesar de, a meu ver, estarem errados.
//Mudanças nas linhas: 36, 97 e 98.
  if(Insere(*it, min(pos + numero_pos, (int)nomes.size())) == -1) return -1;
  if(Remove(pos) == -1) return -1;
