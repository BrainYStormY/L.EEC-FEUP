...
//ToCharacter
  PeopleToCharacterMap = unordered_map<string, unordered_map<string, int>>(); //from TitlePrincipals
...
//ToCharacter
  ...
  {
    PeopleToCharacterMap[character][principal.nconst]++;  //increment principal's number of plays as the character
  }
}
...
//encontradas, organizado alfabeticamente. Em caso de erro, retorna um vetor vazio.
  ...
  unordered_set<TitlePrincipals, hashPerson> pplNconst; //set to store unique principals
  ...
  for(auto p = people.first; p != people.second; p++) //iterate through all people of the series
  {
    pplNconst.insert(p->second);                                  //add person to pplNconst set
...
//encontrada, ou uma string vazia em caso de erro.
  ...
  for(const auto& entry : PeopleToCharacterMap){
